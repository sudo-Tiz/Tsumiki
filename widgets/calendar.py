...
class CalendarWidget(EventBox):
    ...
    def __init__(self, config: BarConfig):
        ...
        self.prev_month_button = Button(
            name="chevron",
            child=Label(
                markup="<span font-family='tabler-icons'>&#xea60;</span>",
            ),
        )
        ...
        self.next_month_button = Button(
            name="chevron",
            child=Label(markup="<span font-family='tabler-icons'>&#xea61;</span>"),
        )
    ...
    def get_weekday_initials(self):
        ...
        return [
            datetime(2021, 1, i).strftime("%a")[0].upper() for i in range(3, 10)
    ...
    def reset_calendar(self):
        for revealer in self.revealers:
            ...
            if any(
                label.get_name() == "current-day" for label in week_box.get_children()
            ):
                revealer.set_reveal_child(True)
            else:
                revealer.set_reveal_child(False)
    ...
    def on_unhover(self, widget, event):
        for revealer in self.revealers:
            if (
                ...
                week_box = revealer.get_child()
                if any(
                    label.get_name() == "current-day"
                    for label in week_box.get_children()
                ):
                    revealer.set_reveal_child(True)
                else:
                    revealer.set_reveal_child(False)
