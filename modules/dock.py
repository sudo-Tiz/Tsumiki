...
# credits to https://github.com/S4NKALP for the original code
class Dock(Window):
    """A widget to display the dock panel."""
    ...
    def check_hide(self, *args):
        """Show the dock immediately in an empty workspace;
        otherwise, hide if needed."""
    ...
    def create_button(self, app, instances):
        ...
        if instances:
            items.append(
                Image(
                    pixbuf=self.icon.get_icon_pixbuf(
                        "filename-dot-amarok", self.config["icon_size"]
                    ),
